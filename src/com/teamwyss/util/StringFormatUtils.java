...
public class StringFormatUtils {
	 ...
	 * @return Numeric only string.
	 */
	public String removeNonAlphaNumeric(String sIn) {
		return removeNonAlphaNumeric(sIn, "");
	}
	 ...
	 * @return Numeric only string.
	 */
	public String removeNonAlphaNumeric(String sIn, String sReplaceWith) {
		String sLower = sIn.toLowerCase();
		StringBuffer sbOut = new StringBuffer(sIn.length());
		char cThis = 0;
		boolean isLastCharReplaced = false;
		boolean isUseReplaceWith = sReplaceWith.length() > 0;
		for (int iChar = 0; iChar < sIn.length(); iChar++) {
			cThis = sLower.charAt(iChar);
			if ((cThis == 32) // Space
					|| ((cThis >= 97) && (cThis <= 122)) // a-z
					|| ((cThis >= 48) && (cThis <= 57)) // 0-9
			) {
				sbOut.append(sIn.charAt(iChar));
				isLastCharReplaced = false;
			} else if (isUseReplaceWith) {
				if (!isLastCharReplaced) {
					sbOut.append(sReplaceWith);
				}
				isLastCharReplaced = true;
			}
		}
		return sbOut.toString();
	}
	...
	public String getStringExcludingBetween(String sMaster, String sStart, String sEnd) {
		...
		while ((iPosStart != -1) && (iPosEnd != -1)) {
			sMaster = sMaster.substring(0, iPosStart) + sMaster.substring(iPosEnd + sEnd.length());
			iPosStart = sMaster.indexOf(sStart);
			iPosEnd = sMaster.indexOf(sEnd, iPosStart);
		}
		return sMaster;
		*/
	...
	public String removeSingleLineComments(String sSuper) {
		String sStripped = removeSingleLineComments(sSuper, "//");
		sStripped = trim(sStripped);
		return sStripped;
	}

	public String removeSingleLineComments(String sSuper, String sCommentStartMarker) {
		ArrayUtils au = new ArrayUtils();
		if(sSuper.indexOf(sCommentStartMarker) != -1){
			sSuper = au.join(au.splitAtTwoPartSeparator(sSuper, sCommentStartMarker, "\r\n"), "\r\n");
		}
		return sSuper;
	}

	public String formatNumberToLength(int iNum, int iLen) {
		String sNum = "" + iNum;
